...
tmp = '/scratch/gpfs/KNORMAN/aw1907/MindEyeV2/cache'
...
for sub in range(8): #,1,2,3,4,5,6,7]:
    ...
    # Build one consistent voxel mask for this subject and keep index ordering explicit.
    vox_include = copy.deepcopy(nsdgeneral_mask)
    ncsnr_file = f"{nsd_path}/nsddata_betas/ppdata/{subject}/func1pt8mm/betas_fithrf_GLMdenoise_RR/ncsnr.nii.gz"
    ncsnr = nib.load(ncsnr_file).get_fdata()
    ncsnr[ncsnr<.15] = np.nan 
    print("voxels left:", len(vox_include[vox_include>0]))
    vox_include[np.isnan(ncsnr)] -= 1 # keep all nsdgeneral voxels even if they are below the threshold
    vox_include[vox_include<0] = 0
    print("voxels left after ncsnr thresholding:", len(vox_include[vox_include>0])) # subj01 = 49329

    vox_include_bool = vox_include.flatten().astype(bool)
    voxel_indices_flat = np.where(vox_include_bool)[0].astype(np.int32)
    coords_ijk = np.column_stack(np.unravel_index(voxel_indices_flat, vox_include.shape)).astype(np.int16)
    print("coords_ijk shape:", coords_ijk.shape)
    ...
    with h5py.File(f'betas_{subject}.hdf5', 'w') as f:
        ...
        f.create_dataset('voxel_indices_flat', data=voxel_indices_flat)
        f.create_dataset('coords_ijk', data=coords_ijk)
    print(f"saved betas_{subject}.hdf5 with coords_ijk")
        ...
        for i in range(len(behav)):
            ...
            for j in range(1):
                ...
                if jj >= 0:
                    ...
                    assert (int(behav.iloc[jj]['SESSION'])-1)*750 + jj < ntrials
