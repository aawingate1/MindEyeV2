...
# NOTE: stripped notebook magic:     %load_ext autoreload 
    # this allows you to change functions in models.py or utils.py and have this notebook automatically update with your revisions
# NOTE: stripped notebook magic:     %autoreload 2 
...
if not os.path.exists(f'{outdir}/{tag}.pth'):
    fallback_outdir = os.path.abspath(f'train_logs/{model_name}')
    if os.path.exists(f'{fallback_outdir}/{tag}.pth'):
...
unique_test_images = np.unique(test_images_idx)
if max_images is not None:
    unique_test_images = unique_test_images[:max_images]

with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.float16):
    for batch in tqdm(range(0,len(unique_test_images),minibatch_size)):
        uniq_imgs = unique_test_images[batch:batch+minibatch_size]
        voxel = None
