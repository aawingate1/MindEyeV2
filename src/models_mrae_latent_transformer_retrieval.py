...
class MRAELatentTransformerRetrievalModel(BaseMRAEVariantModel):
    ...
    def forward_from_manifold(self, manifold: torch.Tensor):
        ...
        aux_clip_tokens = self.aux_out(queries)
...
def add_model_specific_args(parser: argparse.ArgumentParser):
    ...
    parser.add_argument("--token_aux_scale", type=float, default=0.15)
    parser.add_argument("--global_scale", type=float, default=0.35)
    parser.set_defaults(projector_tune_mode="full", projector_lr_scale=0.03)


def build_model(args):
    return MRAELatentTransformerRetrievalModel(args)
